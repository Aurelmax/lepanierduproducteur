...
interface FormData {
  ...
  selectedDistributionPoint: DistributionPoint | null;
}
...
export default function SimpleCheckout({ onClose }: SimpleCheckoutProps) {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    comments: '',
    paymentMethod: '',
    selectedDistributionPoint: null,
  });
  ...
  // Calculer les frais de livraison selon le point sélectionné
  const getDeliveryFee = () => {
    if (formData.selectedDistributionPoint) {
      if (formData.selectedDistributionPoint.type === 'livraison_gratuite') {
        return 0;
      } else if (formData.selectedDistributionPoint.type === 'livraison_payante') {
        return formData.selectedDistributionPoint.deliveryInfo?.deliveryCost || 0;
      }
    }
    // Fallback pour l'ancien système
    return formData.deliveryMethod === 'delivery' && total < 15 ? 5.0 : 0;
  };
  ...
  const handleDistributionPointSelect = (point: DistributionPoint) => {
    setFormData(prev => ({ ...prev, selectedDistributionPoint: point }));
    if (errors.selectedDistributionPoint) {
      setErrors(prev => ({ ...prev, selectedDistributionPoint: '' }));
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.selectedDistributionPoint) {
      newErrors.selectedDistributionPoint = 'Veuillez choisir un point de distribution.';
    }
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
          ...
          <div>
            ...
            <div className='space-y-4'>
              ...
              {/* Sélecteur de points de distribution */}
              <div className='mt-8'>
                <DistributionPointSelector
                  selectedPoint={formData.selectedDistributionPoint}
                  onPointSelect={handleDistributionPointSelect}
                  orderAmount={total}
                />
                {errors.selectedDistributionPoint && (
                  <p className='text-red-500 text-xs mt-2'>{errors.selectedDistributionPoint}</p>
                )}
              </div>

              {/* Adresse de livraison si nécessaire */}
              {formData.selectedDistributionPoint?.type !== 'retrait' && (
                <div className='space-y-4 mt-6'>
                  <h4 className='text-lg font-semibold text-accent-dark'>Adresse de livraison</h4>
                  <div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                ...
                <div>
                  ...
                  {errors.deliveryTimeSlot && (
                    <p className='text-red-500 text-xs mt-1'>{errors.deliveryTimeSlot}</p>
                  )}
                </div>
              </div>
          ...
          <div>
            ...
            <div className='bg-gray-50 rounded-lg p-6 mb-6'>
              <div className='space-y-2'>
                ...
                {formData.selectedDistributionPoint && (
                  <div className='text-sm text-gray-600 mt-2 pt-2 border-t border-gray-200'>
                    <div className='font-medium'>Point de distribution :</div>
                    <div>{formData.selectedDistributionPoint.name}</div>
                    <div className='text-xs'>
                      {formData.selectedDistributionPoint.address},{' '}
                      {formData.selectedDistributionPoint.postalCode} -{' '}
                      {formData.selectedDistributionPoint.city}
                    </div>
                  </div>
                )}
